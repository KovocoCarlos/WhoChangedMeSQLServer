...
function Handle-GetDatabases {
    try {
        $q = "SELECT name FROM sys.databases WHERE state_desc = 'ONLINE' AND name NOT IN ('master','tempdb','model','msdb') ORDER BY name"
        $results = Invoke-SqlQuery -Query $q
...
function Handle-GetObjects {
    param($Body)
    try {
        $q = "SELECT s.name AS schema_name, o.name AS object_name, o.type_desc AS object_type, " +
             "s.name + '.' + o.name AS full_name " +
             "FROM sys.objects o JOIN sys.schemas s ON o.schema_id = s.schema_id " +
             "WHERE o.type IN ('U','V','P','FN','IF','TF','TR') AND o.is_ms_shipped = 0 " +
             "ORDER BY o.type_desc, s.name, o.name"
        $results = Invoke-SqlQuery -Database $Body.database -Query $q
...
function Handle-CreateAudit {
    ...
    try {
        ...
        # Create server audit
        $q1 = "IF NOT EXISTS (SELECT 1 FROM sys.server_audits WHERE name = '$auditName') " +
              "BEGIN " +
              "CREATE SERVER AUDIT [$auditName] TO FILE ( FILEPATH = '$path', MAXSIZE = 100 MB, " +
              "MAX_ROLLOVER_FILES = 10, RESERVE_DISK_SPACE = OFF ) " +
...
function Handle-RemoveAudit {
    ...
    try {
        ...
        try {
            $q1 = "IF EXISTS (SELECT 1 FROM sys.database_audit_specifications WHERE name = '$specName') " +
                  "BEGIN " +
                  "ALTER DATABASE AUDIT SPECIFICATION [$specName] WITH (STATE = OFF); " +
                  "DROP DATABASE AUDIT SPECIFICATION [$specName]; " +
                  "END"
            Invoke-SqlNonQuery -Database $db -Query $q1
        }
        catch { }
...
function Handle-ReadAudit {
    ...
    try {
        ...
        $q = "SELECT event_time, action_id, " +
             ...
             "[statement], client_ip, application_name, host_name, " +
             "session_id, transaction_id, class_type " +
             "FROM sys.fn_get_audit_file('$path\*.sqlaudit', DEFAULT, DEFAULT) " +
             "WHERE ( (object_name = '$obj' AND schema_name = '$schema') " +
             "OR ( class_type IN ('OB','SC') AND [statement] LIKE '%$obj%' ) ) " +
             "ORDER BY event_time DESC"

        $results = Invoke-SqlQuery -Database "master" -Query $q
...
try {
    ...
    Write-Host "  Try: .\Start-WhoChangedThat.ps1 -Port 9000" -ForegroundColor Yellow
    exit 1
}
...
try {
    while ($listener.IsListening) {
        ...
        try {
            ...
            Write-Host "  [ERROR] $urlPath : $($_.Exception.Message)" -ForegroundColor Red
            try {
